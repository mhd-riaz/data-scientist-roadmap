...
- [ ] Profile report exists and is referenced by the decisions made below it.
- [ ] Cleansing is deterministic: running it twice on the same input produces
      byte-identical output.
- [ ] Rejection log accounts for **100%** of the difference between input and
      output counts, by reason code.
...
- [ ] Taxonomy has **8–14 classes**; every class has **≥ 300 weakly-labelled
      articles** in the snapshot. *(provisional — if unmet, coarsen the taxonomy
      or collect longer; do not proceed with starved classes)*
...
- [ ] Near-duplicate detection evaluated on **≥ 200 hand-labelled pairs**;
      precision **≥ 0.90**. *(provisional)*
- [ ] Splits are grouped by story cluster. **Zero** story clusters span two
      splits — assert this in a test, do not check it by eye.
- [ ] Splits are temporal: every test article is published after every training
      article.
- [ ] Snapshot rebuilds byte-identically from `(git SHA, cleaning_version, seed)`.
- [ ] Data card records: article count, class distribution, date range, sources,
      cleaning version, known limitations.
...
1. **Taxonomy size and single- vs multi-label.** Blocks Phase 2. Highest-risk
   open item.
2. **Corpus sufficiency.** Current collection rate versus articles needed per
   class. Answer during the Phase 2 profile; it may force a coarser taxonomy.
...
6. **Handling of summary-only sources** (NDTV and any other permanently
   `blocked` source). Restrict the corpus, model headline-only separately, or
   train everything on `title + summary` so the field is level. Decide in Phase 2.
